...
// ═══════════════════════════════════════════════════════════════════════════
// TAB: QR CODE GENERATOR (uses qrcode-generator library via CDN)
// ═══════════════════════════════════════════════════════════════════════════

// Load qrcode-generator from CDN dynamically
let qrcodeLib = null;
const loadQRLib = () => new Promise((resolve) => {
  ...
  script.onload = () => {
    // qrcodejs renders to DOM, use a different lib
    resolve(null);
  };
  document.head.appendChild(script);
});

// Pure JS QR using qrcode-generator (loaded from cdnjs)
const loadQRGenerator = () => new Promise((resolve) => {
  if (window._qrgen) { resolve(window._qrgen); return; }
  const script = document.createElement("script");
  script.src = "https://cdnjs.cloudflare.com/ajax/libs/qrcode-generator/1.4.4/qrcode.min.js";
  script.onload = () => { window._qrgen = window.qrcode; resolve(window._qrgen); };
  script.onerror = () => resolve(null);
  document.head.appendChild(script);
});

const buildQRSVGFromLib = (qr, darkColor, lightColor, transparent, moduleSize=6) => {
  const count = qr.getModuleCount();
  const margin = moduleSize * 4;
  const totalSize = count * moduleSize + margin * 2;
  let rects = "";
  for (let r = 0; r < count; r++) {
    for (let c = 0; c < count; c++) {
      const dark = qr.isDark(r, c);
      if (dark) {
        rects += `<rect x="${margin + c*moduleSize}" y="${margin + r*moduleSize}" width="${moduleSize}" height="${moduleSize}" fill="${darkColor}"/>`;
      } else if (!transparent) {
        rects += `<rect x="${margin + c*moduleSize}" y="${margin + r*moduleSize}" width="${moduleSize}" height="${moduleSize}" fill="${lightColor}"/>`;
      }
    }
  }
...
function TabQR() {
  ...
  useEffect(() => {
    loadQRGenerator().then(lib => {
      if (lib) setLibReady(true);
      else setError("No se pudo cargar la librería QR. Verifica tu conexión.");
    });
  }, []);

  useEffect(() => {
    if (!libReady || !text.trim()) { setSvgCode(""); setError(""); return; }
    try {
      const qr = window._qrgen(0, "M"); // type 0 = auto, ECC M
      qr.addData(text);
      qr.make();
      setSvgCode(buildQRSVGFromLib(qr, darkColor, lightColor, transparent));
      setError("");
    } catch(e) {
      setError("Texto demasiado largo. Intenta con menos de 300 caracteres.");
      setSvgCode("");
    }
  }, [text, darkColor, lightColor, transparent, libReady]);

  const previewSvg = (() => {
    if (!svgCode || !libReady || !text.trim()) return "";
    if (transparent) {
      try {
        const qr = window._qrgen(0, "M");
        qr.addData(text); qr.make();
        return buildQRSVGFromLib(qr, darkColor, "#ffffff", false);
      } catch { return svgCode; }
    }
    return svgCode;
  })();
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,alignItems:"start"}}>
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        <div style={{background:"#141414",border:"1px solid #222",borderRadius:16,padding:20}}>
          ...
          <textarea style={{...inp,minHeight:100}} value={text} onChange={e=>setText(e.target.value)}
            placeholder={"URL, texto, email, teléfono..."} maxLength={500}/>
          <div style={{display:"flex",justifyContent:"space-between",marginTop:6}}>
            <span style={{fontSize:11,color:error?"#ff6b6b":"#555"}}>{error||(!libReady?"Cargando librería...":(`${text.length} caracteres`))}</span>
        ...
        <div style={{background:"#141414",border:"1px solid #222",borderRadius:16,padding:20}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{fontSize:11,color:"#555",letterSpacing:"0.15em"}}>COLOR DE FONDO</div>
            <button style={{display:"flex",alignItems:"center",gap:6,background:"transparent",border:"none",cursor:"pointer",padding:0}} onClick={()=>setTransparent(t=>!t)}>
              <div style={{width:32,height:18,borderRadius:9,background:transparent?"rgba(200,245,102,0.2)":"#2a2a2a",border:`1px solid ${transparent?"#c8f566":"#333"}`,position:"relative",transition:"all 0.2s"}}>
                <div style={{position:"absolute",top:2,left:transparent?14:2,width:12,height:12,borderRadius:"50%",background:transparent?"#c8f566":"#555",transition:"all 0.2s"}}/>
              </div>
              <span style={{fontSize:10,color:transparent?"#c8f566":"#555",letterSpacing:"0.08em"}}>TRANSPARENTE</span>
        ...
        <div style={{background:"#141414",border:"1px solid #222",borderRadius:16,overflow:"hidden",marginBottom:12}}>
          ...
          <div style={{padding:32,display:"flex",alignItems:"center",justifyContent:"center",minHeight:240,background:"repeating-conic-gradient(#1a1a1a 0% 25%, #141414 0% 50%) 0 0 / 20px 20px"}}>
            {svgUrl
              ...
              : <div style={{textAlign:"center",color:"#333",fontSize:13}}>{libReady ? <>Escribe algo arriba<br/><span style={{fontSize:11,color:"#2a2a2a"}}>para generar el QR</span></> : "Cargando..."}</div>
            }
          </div>
        </div>
        {svgCode && (
